    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative z-10 bg-gradient-to-b from-[#0e1e4a] to-[#0a1228] border border-[#1e3a6e] rounded-2xl p-8 max-w-sm w-full flex flex-col items-center gap-4 text-center shadow-2xl">
                ...
                <p className="text-[#8a9fc8] text-sm leading-relaxed">Selling cards is coming soon! Stay tuned for updates.</p>
                <SvgButton
                    onClick={onClose}
                    className="w-full h-[60px] mt-2"
                >
                    Go back
                </SvgButton>
            </div>
...
function SellConfirmButton({ onDone }: { onDone: () => void }) {
    ...
    return (
        <>
            ...
            {showUpcoming && <UpcomingModal onClose={() => { setShowUpcoming(false); onDone(); }} />}
        </>
    );
}
...
interface SellModalProps {
    isOpen: boolean;
    onClose: () => void;
    card?: { name: string; info: string; rarity?: string; set?: string; description?: string; image?: string; };
}

export default function SellModal({ isOpen, onClose, card }: SellModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 py-10 pb-15 overflow-y-scroll">
            ...
            <div className="relative z-10 bg-transparent lg:bg-[#151932] rounded-2xl p-8 px-10 pt-15 mt-20 max-w-3xl w-full shadow-2xl flex flex-col md:flex-row gap-6 ">
                <button onClick={onClose} className="absolute top-3 right-4 w-8 h-8 rounded-full bg-white/10  flex items-center justify-center text-white hover:bg-[#1e3a6e] transition-colors">
                    <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M1 1L11 11M11 1L1 11" stroke="white" strokeWidth="1.8" strokeLinecap="round" /></svg>
                </button>
                ...
                <div className="flex-shrink-0 flex items-center justify-center">
                    <div className="w-auto h-80 lg:h-100 rounded-xl overflow-hidden shadow-lg">
                        {card?.image ? (
                            <Image src={card.image} alt={card.name || "Card"} width={200} height={300} className="object-cover w-full h-full" draggable="false" />
                        ) : (
                            <div className="w-full h-full flex flex-col bg-gradient-to-b from-[#1a3a8c] to-[#0d2060] p-3 gap-2">
                                <div className="flex justify-between items-center">
                                    <div className="w-7 h-7 rounded-full bg-yellow-500 flex items-center justify-center text-black font-bold text-xs">1</div>
                                ...
                                <div className="bg-[#0a1a4a]/80 rounded-lg p-2">
                                    <p className="text-yellow-300 font-bold text-xs text-center">{card?.name || "Lorem Ipsum"}</p>
                                    <p className="text-white/50 text-[9px] text-center truncate">Lorem ipsum dolor amet</p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex-1 flex flex-col gap-2 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                        <div>
                            <h2 className="text-white font-bold text-xl leading-tight">{card?.name || "Card title"}</h2>
                            <p className="text-[#8a9fc8] text-sm">{card?.info || "[Card Info]"}</p>
                        </div>
                        <span className={`text-xs font-bold px-3 py-1 rounded-full flex-shrink-0 ${rarityClass}`}>{rarity}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-white text-sm mt-0.5">{card?.set || "One Piece Pack"}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-white text-sm mt-0.5">{card?.info || "[Card Info]"}</p>
                    </div>
                    ...
                    <div className="bg-[#0F1329] border border-[#1e3a6e]/40 rounded-xl p-4 h-40">
                        ...
                        <p className="text-[#8a9fc8] text-xs leading-relaxed">
                            {card?.description || "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."}
                        </p>
                    </div>
                    ...
                    <SellConfirmButton onDone={onClose} />
                </div>
            </div>
        </div>
    );
